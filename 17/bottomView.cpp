...
// last node on every vertical order is our bottom view
vector<int> bottomView(TreeNode* root, vector<int> &ans){
    ...
    while(!q.empty()){
        ...
        int line = it.second;
        // updates every time to get the last node on every vertical order
        mp[line] = node->val;
